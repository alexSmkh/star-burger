...
### [Демка тут](https://dvmn-prjcts.fun)
...
- `ROLLBAR_TOKEN` - для работы сервиса логирования ошибок. [Зарегистрируйтесь](https://rollbar.com/), создайте проект и скопируйте ключ.
- `ROLLBAR_NAME` - название проекта в Rollbar
- `DB_URL` - url вашей базы данных.

Для обновления проекта на проде используйте скрипт `deploy.sh`. Он подтянет изменения из репозитория, выполнит миграции,
обновит
зависимости, сбилдит фрот и статику, перезапустит сервер. Перед его запуском установите [jo](https://github.com/jpmens/jo): `apt install jo`

Для запуска можно указать путь к проекту, дефолтное
значение которого: `/opt/star-burger/`:
```shell
./deploy.sh

# если путь к проекту не совпадает с дефолтным:
PROJECT_PATH='/full/project/path/' ./deploy.sh
```
